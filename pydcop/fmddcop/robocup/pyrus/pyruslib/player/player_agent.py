...
from pydcop.fmddcop.robocup.pyrus.pyruslib.action.kick_table import KickTable
from pydcop.fmddcop.robocup.pyrus.base.decision import get_decision
...
from pydcop.fmddcop.robocup.pyrus.pyruslib.player.soccer_agent import SoccerAgent
from pydcop.fmddcop.robocup.pyrus.pyruslib.player.world_model import WorldModel
from pydcop.fmddcop.robocup.pyrus.pyruslib.network.udp_socket import IPAddress
from pydcop.fmddcop.robocup.pyrus.pyruslib.player_command.player_command import PlayerInitCommand, PlayerByeCommand
from pydcop.fmddcop.robocup.pyrus.pyruslib.player_command.player_command_body import PlayerTurnCommand, PlayerDashCommand, PlayerMoveCommand, \
    PlayerKickCommand, PlayerTackleCommand
from pydcop.fmddcop.robocup.pyrus.pyruslib.player_command.player_command_support import PlayerDoneCommand, PlayerTurnNeckCommand
...
class PlayerAgent(SoccerAgent):
    class Impl:
        ...
        def send_init_command(self):
            ...
            com = PlayerInitCommand(TEAM_NAME, 15, False)
            # TODO set team name from config
            self._agent._full_world._team_name = TEAM_NAME
    ...
    def handle_start(self):
        ...
        if not self._client.connect_to(IPAddress('localhost', 6000)):
            print("ERROR failed to connect to server - handle_start")
    ...
    def run(self):
        ...
        while True:
            ...
            while True:
                ...
                elif time.time() - last_time_rec > 20:
                    self._client.set_server_alive(False)
                    break
                message_count += 1
